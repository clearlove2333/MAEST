...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        # u_p, v_p = torch.tensor(u + v), torch.tensor(v + u)
        # df = pd.DataFrame({'u': u, 'v': v})
        # df.to_csv('/zhupengfei/STExperiment/MAEST/debug/u_v.csv', index=False)

        # df2 = pd.DataFrame({'u': u_p.numpy(), 'v': v_p.numpy()})
        # df2.to_csv('/zhupengfei/STExperiment/MAEST/debug/u_v_p.csv', index=False)
        # break
        ...
        # for epoch in range(max_epoch):
            ...
            if (epoch + 1) % 50 == 0:
                ...
                plt.savefig(os.path.join(result_path, str(epoch) + '.pdf'))
